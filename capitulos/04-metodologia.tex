...
legibilidade validada para o portugu\^es -- na tradi\c{c}\~ao inaugurada
pela adapta\c{c}\~ao do \'Indice de Flesch de
\textcite{martins1996readability}, com coeficientes re-derivados de
forma independente pelo software ALT \autocite{moreno2023alt} -- e
ancorado nas caracter\'isticas do p\'ublico de perfil popular
identificadas no diagn\'ostico do Cap\'itulo 5 (baixa familiaridade com
terminologia de renda fixa), de
...
Pr\'atica convergente aparece em \textcite{wang2025omnieval}, cujo
framework de avalia\c{c}\~ao autom\'atica de RAG financeiro combina
m\'etricas baseadas em LLM com anota\c{c}\~ao humana e ajuste fino do
modelo avaliador especificamente para garantir a confiabilidade do
julgamento autom\'atico -- refor\c{c}ando, por via emp\'irica distinta, a
mesma cautela recomendada por \textcite{bavaresco2025}.
...
Reconhece-se, contudo, evid\^encia em dire\c{c}\~ao distinta:
\textcite{zhu2025}, em QA educacional financeiro com RAG, reporta que o
ganho de acur\'acia obtido por um agente de cr\'itica (Expert Reviewer)
superou o ganho da etapa de recupera\c{c}\~ao de evid\^encia isolada --
achado que n\~ao invalida a segrega\c{c}\~ao c\'alculo/gera\c{c}\~ao
proposta em H1, mas sugere que a adi\c{c}\~ao de uma etapa de revis\~ao
multiagente \'e uma dire\c{c}\~ao de aprimoramento n\~ao explorada no
escopo desta disserta\c{c}\~ao.
...
Complementarmente, a avalia\c{c}\~ao da etapa de gera\c{c}\~ao seguir\'a o
desenho multidimensional proposto por \textcite{wang2025omnieval} para RAG
financeiro, combinando m\'etricas rule-based (Rouge-L, F1) com m\'etricas
baseadas em LLM voltadas especificamente \`a detec\c{c}\~ao de
alucina\c{c}\~ao e \`a acur\'acia num\'erica -- dimens\~ao diretamente
aplic\'avel \`a condi\c{c}\~ao de controle desta pesquisa (LLM sem m\'odulo
determin\'istico), na qual o erro de c\'alculo tribut\'ario \'e o risco
central a ser medido.
...
% NOTA: a matriz de adequacao produto x perfil (CVM 30/2021 art. 3 +
% caracterizacao qualitativa de risco por produto) foi construida e
% fechada no Anexo III (anexo-3-instrumento-avaliacao.tex, secao
% "Matriz de adequacao produto x perfil"), incluindo a logica de CDB
% acima do limite do FGC (Indice de Basileia + rating de agencias, pior
% sinal disponivel, com retirada de rating tratada como sinal de risco
% elevado). As decisoes de citacao ja foram executadas: Foerster et al.
% (2017) citado como apoio ao abandono do especialista ao vivo; Jacobs
% et al. e Choi (2022) nao citados (seguem so como precedente
% metodologico registrado em bibliografia/literatura-suitability-
% benchmark.md, sem sustentar conteudo numerico especifico desta
% dissertacao, que mede apenas renda fixa).
