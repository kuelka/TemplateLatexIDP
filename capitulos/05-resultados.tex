...
Duas edi\c{c}\~oes do Raio X do Investidor Brasileiro s\~ao utilizadas
neste trabalho, por cobrirem informa\c{c}\~oes complementares e n\~ao
sobrepostas: a 8\textsuperscript{a} edi\c{c}\~ao \autocite{anbima2025}
\'e a \'unica das duas a publicar o detalhamento demogr\'afico completo
por perfil (idade, renda, escolaridade, regi\~ao, classe social,
estresse financeiro), estrutura que a 9\textsuperscript{a} edi\c{c}\~ao
n\~ao reproduz -- por isso fundamenta a segmenta\c{c}\~ao comportamental
em quatro perfis j\'a utilizada para calibrar as personas sint\'eticas
do Anexo~\ref{ane:personas}. A 9\textsuperscript{a} edi\c{c}\~ao
\autocite{anbima2026}, publicada mais de um ano depois, confirma a
estabilidade desses quatro perfis (varia\c{c}\~ao de no m\'aximo 1
ponto percentual em rela\c{c}\~ao \`a 8\textsuperscript{a} edi\c{c}\~ao)
e acrescenta ao diagn\'ostico um achado in\'edito, indispon\'ivel na
edi\c{c}\~ao anterior: o uso emergente de assistentes de
intelig\^encia artificial como canal de informa\c{c}\~ao sobre
investimentos (ver Se\c{c}\~ao~\ref{sec:contextualizacao}). Entre os
dados est\'aveis entre as duas edi\c{c}\~oes, destaca-se que 12\% da
popula\c{c}\~ao brasileira consegue poupar mas mant\'em o dinheiro
parado sem aplicar em nenhum produto financeiro -- o Perfil Economiza
e N\~ao Investe, assim caracterizado j\'a na 8\textsuperscript{a}
edi\c{c}\~ao \autocite{anbima2025} e confirmado na
9\textsuperscript{a} --, evidenciando o funil
poupan\c{c}a--investimento como gargalo estrutural que antecede a
...
da gera\c{c}\~ao de linguagem. Uma terceira barreira \'e de acesso: 12\%
da popula\c{c}\~ao brasileira consegue poupar, mas mant\'em o dinheiro
parado sem aplicar em nenhum produto financeiro -- o Perfil Economiza e
N\~ao Investe (Camada 2) --, evidenciando atrito na transi\c{c}\~ao entre
...
\begin{itemize}
  ...
  \item[] \textit{5.3. Base de conhecimento regulat\'oria RAG -- recall@k e prioriza\c{c}\~ao de norma vigente (objetivo d) [PENDENTE -- corpus normativo fechado (6 de 6 normas verificadas, 6 de 6 PDFs arquivados) em bibliografia/normas-rag-corpus.md]}
  \item[] \textit{5.4. Resultados do backtesting de 12 meses (jun/25--mai/26) [PENDENTE]}
  \item[] \textit{5.5. Avalia\c{c}\~ao t\'ecnica automatizada (objetivo e) [PENDENTE -- resultados da aplica\c{c}\~ao no backtesting; a matriz de adequa\c{c}\~ao produto\texorpdfstring{$\times$}{x}perfil e a l\'ogica de CDB acima do limite do FGC j\'a est\~ao constru\'idas e fechadas no Anexo~\ref{ane:instrumento-avaliacao}]}
\end{itemize}
